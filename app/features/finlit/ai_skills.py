...
def _money(ctx: dict[str, Any], value: float | None) -> str:
    ...
    return f"{meta['symbol']}{value:,.0f}" if meta["decimals"] == 0 else f"{meta['symbol']}{value:,.2f}"
...
class PlanReviewSkill:
    ...
    def can_handle(self, user_text: str, system: str) -> bool:
        t = user_text.lower()
        return any(k in t for k in ("my plan", "budget", "review", "how am i doing", "50/30/20", "split", "spend"))
    ...
    def respond(self, user_text: str, system: str, context: dict[str, Any] | None) -> str:
        ...
        if plan.get("overspend", 0) > 0:
            parts.append(f"First fix: you are {_money(ctx, plan['overspend'])} over your income, so trim wants until it balances.")
        elif sh["savings"] < 0.20:
            gap = (0.20 - sh["savings"]) * plan["income"]
            parts.append(f"Moving {_money(ctx, gap)} from wants to savings each {plan['period'].replace('ly', '')} reaches the 20% target — over a year that is {_money(ctx, gap * (52 if plan['period']=='weekly' else 26 if plan['period']=='biweekly' else 12))}.")
        else:
            parts.append(f"You are saving {_money(ctx, plan['annual']['savings'])} a year at this rate. Next step: name what it is for so it stays untouched.")
        if plan.get("tips"):
...
class GoalSkill:
    ...
    def can_handle(self, user_text: str, system: str) -> bool:
        t = user_text.lower()
        return any(k in t for k in ("goal", "save for", "how long", "weeks", "afford", "reach"))
    ...
    def respond(self, user_text: str, system: str, context: dict[str, Any] | None) -> str:
        ...
        lines = []
        for g in goals[:3]:
            eta = f"reaches {g['target_eta']}" if g.get("target_eta") else "has no weekly amount yet, so no date"
            lines.append(f"• {g['name']}: {_money(ctx, g['contributed'])} of {_money(ctx, g['target'])} ({g['progress']:.0f}%), {_money(ctx, g['remaining'])} to go — at {_money(ctx, g['weekly'])}/week it {eta}.")
...
class DebtSkill:
    ...
    def can_handle(self, user_text: str, system: str) -> bool:
        ...
        if ctx.get("financing"):
            return True
        return any(k in t for k in ("debt", "credit", "loan", "borrow", "interest", "financ", "installment", "instalment", "bnpl", "pay later", "per month", "monthly payment"))
...
class ProgressSkill:
    ...
    def can_handle(self, user_text: str, system: str) -> bool:
        t = user_text.lower()
        return any(k in t for k in ("progress", "score", "learn", "module", "assessment", "improve", "next", "what should i"))
    ...
    def respond(self, user_text: str, system: str, context: dict[str, Any] | None) -> str:
        ...
        sim = prof.get("sim", {})
        msg = [f"Modules completed: {modules_done}/{total}. Assessment: baseline {pre:.0f}% → latest {post:.0f}%" + (f" (+{post - pre:.0f} points)." if post > pre else ".")]
        if sim.get("decisions"):
            msg.append(f"Money Moves: {sim['decisions']} decisions, quality {sim['quality']:.2f}/1.00, wallet {_money(ctx, sim['balance'])}, debt {_money(ctx, sim['debt'])}.")
        weakest = ctx.get("weakest_module")
        if weakest:
            msg.append(f"Next step: retry '{weakest}' — it is your lowest module score and the quiz explains each answer.")
        elif modules_done < total:
            remaining = [m.title for m in content.MODULES.values() if m.module_id not in (ctx.get("completed_ids") or [])]
            if remaining:
                msg.append(f"Next step: the '{remaining[0]}' module takes about four minutes.")
        else:
            msg.append("Next step: take the post-assessment to lock in your gain, then build a real plan in Planner.")
        return "\n\n".join(msg)
...
class ConceptSkill:
    ...
    def can_handle(self, user_text: str, system: str) -> bool:
        t = user_text.lower()
        return any(k in t for k in ("what is", "what's", "explain", "mean", "why", "compound", "emergency fund", "inflation", "diversif"))
    ...
    def respond(self, user_text: str, system: str, context: dict[str, Any] | None) -> str:
        ...
        for module in content.MODULES.values():
            if module.module_id in t or module.title.lower().split()[0] in t:
                return f"{module.title} — {module.topic}. " + " ".join(module.lessons[:2]) + " Next step: open the module and take the 3-question check."
        return "Good question. The short version: know what comes in, decide where it goes before you spend it, keep a buffer for shocks, and let saving run on autopilot. Ask me about your plan, your goals, debt, or any of the six modules."
...
class _ConceptFirst(ConceptSkill):
    ...
    def can_handle(self, user_text: str, system: str) -> bool:
        ...
        return t.startswith(("what is", "what's", "whats", "explain", "define", "what does")) and "my " not in t


LOCAL_SKILLS: list[Any] = [_ConceptFirst(), DebtSkill(), GoalSkill(), PlanReviewSkill(), ProgressSkill(), ConceptSkill()]
...
def build_messages(user_text: str, ctx: dict[str, Any] | None) -> list[AIMessage]:
    ...
    return [AIMessage(role="system", content=system), AIMessage(role="user", content=user_text)]
