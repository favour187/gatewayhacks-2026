...
class FinProfile(UUIDMixin, TimestampsMixin, Base):
    ...
    pre_attempts: Mapped[int] = mapped_column(default=0)
    post_attempts: Mapped[int] = mapped_column(default=0)
...
def upsert_plan(db: Session, user_id: str, payload: dict, result: dict) -> BudgetPlan:
    ...
    if row is None:
        row = BudgetPlan(user_id=user_id)
        db.add(row)
    row.input_json = json.dumps(payload)
